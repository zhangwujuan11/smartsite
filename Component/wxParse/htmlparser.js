...
module.exports = function (c, d) {
  ...
  for (
    ...
    if (((p = !0), h.last() && l[h.last()]))
      (c = c.replace(
        new RegExp("([\\s\\S]*?)</" + h.last() + "[^>]*>"),
        function (e, t) {
          return (
            (t = t.replace(
              /<!--([\s\S]*?)-->|<!\[CDATA\[([\s\S]*?)]]>/g,
      ...
      (0 == c.indexOf("\x3c!--")
        ? (f = c.indexOf("--\x3e")) >= 0 &&
          (d.comment && d.comment(c.substring(4, f)),
          (c = c.substring(f + 3)),
          (p = !1))
        : 0 == c.indexOf("</")
        ? (u = c.match(t)) &&
          ((c = c.substring(u[0].length)), u[0].replace(t, v), (p = !1))
        : 0 == c.indexOf("<") &&
          (u = c.match(e)) &&
          ((c = c.substring(u[0].length)), u[0].replace(e, g), (p = !1)),
      p)
    ) {
      f = c.indexOf("<");
      for (var m = ""; 0 === f; )
        (m += "<"), (f = (c = c.substring(1)).indexOf("<"));
      (m += f < 0 ? c : c.substring(0, f)),
        (c = f < 0 ? "" : c.substring(f)),
        d.chars && d.chars(m);
    }
